...
from core.installer_constants import PERSISTABLE_ENGINES as _PERSISTABLE_ENGINES
...
class OnboardingState:
    ...
    version: int
    engine: str  # "mlx" | "ollama" | "gguf" | "local" (see PERSISTABLE_ENGINES;
                 # "embedder" is download-only and never persisted here)
    ...
    def load(cls) -> OnboardingState | None:
        ...
        try:
            ...
            return None
        # MC-022/MC-043: a hand-edited or legacy file could carry an engine
        # that apply_to_env() cannot resolve. Treat it as "not onboarded"
        # (graceful fallback) rather than returning a poisoned state that
        # crashes the next startup with an uncaught KeyError.
        if state.engine not in _PERSISTABLE_ENGINES:
            logger.warning(
                "onboarding_state: non-persistable engine %r — ignoring state",
                state.engine,
            )
            return None
        return state
    ...
    def save(
        ...
        *,
        ...
        """
        if engine not in _PERSISTABLE_ENGINES:
    ...
    def apply_to_env(self) -> None:
        ...
        if self.engine == "local":
            ...
            # Defense-in-depth: a `.get()` with an explicit ValueError keeps a
            # non-resolvable engine from blowing up startup with an opaque
            # KeyError (MC-022/MC-043). save()/load() already guard against
            # this, so reaching here means a state built bypassing them.
            resolver_key = _ENGINE_TO_RESOLVER_KEY.get(self.engine)
            if resolver_key is None:
                raise ValueError(f"non-resolvable engine: {self.engine!r}")
            ...
            os.environ["NEXE_MODEL_ENGINE"] = resolver_key
