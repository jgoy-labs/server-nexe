"""Anti-regression scenario — `VectorIndex` post-init failure defaults.
...
design decision: dev will do a fix of the type `assert self._client is not None`
...
dev chooses:** post-init failure → public operations return defaults without
crash, NEVER raise.

Pre-fix (HEAD `30eb2a6`): contract is already fulfilled thanks to the `_available` guard.
Post-fix: must continue to be fulfilled. If dev breaks the guard (e.g., removes
...
def test_vector_index_index_empty_entries_returns_zero(tmp_path: Any) -> None:
    ...
    23 callsites. If dev simplifies the guard `if not self._available or not entries`
