"""Anti-regression scenario — `AsyncEmbedder._model` lazy-load guard.
...
design decision (scenario): dev introduces a guard `assert self._model
...
2. **TDD post-fix:** when `_model is None` and `_encode_sync` is invoked, it must
   ...
   explicit contract of the scenario fix — pre-fix is AttributeError.
...
def test_encode_sync_signature_pinned() -> None:
    ...
    assert list(sig.parameters.keys()) == ["self", "text", "normalize"], (
        ...
        "scenario fix must maintain the external signature."
    )
...
def test_async_embedder_model_starts_none() -> None:
    ...
    try:
        assert embedder._model is None, (
            ...
            "lazy-load invariant broken (scenario loses its rationale)."
        )
    finally:
