"""Anti-regression scenario — `api/documents.py` callable contract.
...
design decision (scenario): dev will update the signatures to
...
Post-fix: must continue to be fulfilled. If dev inadvertently changes the body to
...
def test_store_document_is_coroutine_function() -> None:
    """`store_document` is async — scenario fix must NOT change it to sync."""
...
def test_store_document_signature_keeps_generate_embedding_param() -> None:
    """Pins the named parameter `generate_embedding`. If dev renames or
...
async def test_store_document_awaits_async_generate_embedding() -> None:
    ...
    and obtain the embedding via `await`. If dev changes the body to sync, this
    ...
    assert embedding_calls == ["contracte scenario"], (
        "The async embedder was not called exactly once — scenario fix has "
