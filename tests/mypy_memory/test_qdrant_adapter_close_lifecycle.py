"""Anti-regression scenario — `QdrantAdapter` use-after-close lifecycle.
...
design decision (, option A): dev introduces a private helper
...
That is why the test is marked `xfail(strict=True)` — dev will remove it at surgery
(scenario commit, position 13/13).
"""
...
def test_qdrant_adapter_close_method_exists() -> None:
    ...
    assert hasattr(QdrantAdapter, "close"), (
        "QdrantAdapter.close() has disappeared — breaks the scenario premise."
    )
...
def test_qdrant_adapter_protocol_methods_present() -> None:
    ...
    If dev removes/renames any of these, all external callers of the Protocol
    would break — out-of-scope for scenario (which only touches the internal guard)."""
    from memory.embeddings.adapters.qdrant_adapter import QdrantAdapter

    for method in ("add_vectors", "search", "delete", "health"):
        assert hasattr(QdrantAdapter, method), (
            f"QdrantAdapter.{method}() has disappeared — dev has exceeded the cluster scope."
        )
