"""Tests for plugins/llama_cpp_module/health.py — coverage gaps."""
import asyncio
import pytest
...
class TestGetHealthInEventLoop:
    ...
    def test_initialized_returns_healthy(self):
        mock_meta = MagicMock()
        mock_meta.name = "llama_cpp_module"
        ...
        with patch("plugins.llama_cpp_module.manifest.get_module_instance", return_value=mock_module):
            from plugins.llama_cpp_module.health import get_health
        ...
        assert result["module"] == "llama_cpp_module"
...
class TestGetHealthOutsideEventLoop:
    ...
    def test_delegates_to_async_health_check(self):
        ...
        mock_health_result.to_dict.assert_called_once()
