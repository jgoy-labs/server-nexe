"""Anti-regression for `_stream_kwargs` chat_cli (, refactor).

Covers mypy findings #20, #23, #25, #27. Mypy
...
The dev fix (scenario) explicitly annotates `_stream_kwargs: dict[str, Any] = {}`.
...
If dev inadvertently touches the client signature (out-of-scope), this test
...
def test_stream_kwargs_unpack_pattern_binds() -> None:
    ...
    Pre-fix passes (client signature already accepts keyword-only). Post-fix dev
