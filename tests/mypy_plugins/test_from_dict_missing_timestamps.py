...
Description: Blind tests — scenario (session_manager.py from_dict arg-type).

scenario: ChatSession.from_dict() receives session dicts deserialized from disk.
...
Post-fix contract (dev): from_dict() does NOT crash with absent keys and returns
...
def test_from_dict_missing_created_at_does_not_raise():
    """TDD scenario: from_dict() with 'created_at' absent must NOT crash.
...
def test_from_dict_missing_last_activity_does_not_raise():
    """TDD scenario: from_dict() with 'last_activity' absent must NOT crash.
...
def test_from_dict_complete_dict_preserved():
    """Anti-regression scenario: from_dict() with all keys present must NOT change.
