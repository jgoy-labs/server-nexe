...
Post-fix contract (dev): if module_manager=None → HTTPException(503).
...
def test_server_state_module_manager_is_optional_none_by_default():
    """Anti-regression scenario: ServerState.module_manager is Optional, None by default.

    Pins that post-fix dev does not harden module_manager as Required (non-None).
    The attribute must continue to be initialised to None in __init__ (set at
    initialize()/lifespan). If dev changes the default to a non-None value,
    ...
    assert hasattr(state, "module_manager"), (
        ...
        "collateral refactor scenario should not remove it."
    )
    assert state.module_manager is None, (
        ...
        "If post-fix dev changes it to non-None, it breaks the startup contract."
    )


# ──────────────────────────────────────────────────────────────────────────────
# Fixtures for the HTTP test (scenario xfail)
# ──────────────────────────────────────────────────────────────────────────────
...
def test_chat_endpoint_module_manager_none_returns_503():
    """TDD scenario: POST /chat with module_manager=None must return HTTP 503.
    ...
    Post-fix (dev): explicit guard → HTTPException(503, 'Service unavailable').
    """
    ...
    assert r.status_code == 503, (
        f"Expected HTTP 503 (module_manager=None, dev guard), "
