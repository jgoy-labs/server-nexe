...
- Routine dependency updates: click 8.4.2, slowapi 0.1.10, filelock 3.32.3, platformdirs 4.11.3, pypdf 6.16.1, pydantic 2.13.4, pydantic-settings 2.15.0, python-dotenv 1.2.3, structlog 26.1.0, pytest 9.1.1 (test tooling), and uvicorn 0.52.4. structlog uses CalVer, so 25.5.0 → 26.1.0 is one release, not a breaking major. pydantic 2.10.6 → 2.13.4 is 22 releases (Mar 2025–May 2026); the package itself is pure-python and its pinned `pydantic-core==2.46.4` publishes a `win_arm64` wheel, so the Windows pin set moves with the base one. uvicorn 0.34.3 → 0.52.4 is 24 releases in between (Jun 2025–Aug 2026); it is `py3-none-any`, so the Windows pin moves with the base. Windows stays on plain `uvicorn==` without `[standard]`: `httptools` 0.8.0 has no `win_arm64` wheel. Not a security bump (0 OSV advisories at either end). All other listed packages ship pure-python wheels.
...
- **Adaptació de l'idioma de resposta al del missatge (multi-idioma global)** — el motor ara detecta l'idioma de cada missatge de l'usuari i hi respon, en lloc d'ancorar-se a l'idioma d'instal·lació (`NEXE_LANG`). Detecció amb **`lingua`** (75 idiomes, offline —models al wheel—, precisa en text curt i idiomes propers com ca/es/pt/fr/it, on `langdetect` fallava: classificava el castellà curt com a francès). El prompt base es manté en ca/es/en (amb les regles de memòria) o cau a l'anglès per a la resta d'idiomes, i s'hi **prepèn una directiva `[CRITICAL INSTRUCTION]` (en anglès, amb el nom de l'idioma explícit) + un recordatori al final** (els models petits 4B/9B són sensibles a la _recency_). Cobreix UI (`/ui/chat`) i API (`/v1/chat/completions`) amb lògica compartida a `core/lang_detect.py`. _Fallback_ a `NEXE_LANG` en missatges curts/ambigus o codi. Validat en viu amb Gemma3:4b (castellà curt→castellà, alemany→alemany, amb instal·lació en català). Tests: `tests/core/test_lang_detect.py` (19 casos). (2026-05-29)
- **`ollama`: normalització de `base_url` del client** — `resolve_base_url()` ara gestiona adreces de _bind_ (`OLLAMA_HOST=0.0.0.0` / `::`), esquema absent i port absent, normalitzant-les a una URL connectable (`http://127.0.0.1:11434`). Abans, `OLLAMA_HOST=0.0.0.0` (configuració habitual perquè Ollama escolti a totes les interfícies) produïa una URL sense protocol que feia petar httpx (_"Request URL is missing an 'http://' or 'https://' protocol"_), obria el circuit breaker d'Ollama i forçava el _fallback_ silenciós a MLX. Hosts remots, `https://` i ports no estàndard es preserven intactes. Tests: `tests/plugins/ollama_module/unit/test_resolve_base_url.py` (16 casos). (2026-05-29)
...
- **Formal STRIDE threat model** (new `THREAT_MODEL.md` at repo root + `knowledge/{en,ca,es}/THREAT_MODEL.md`; `SECURITY.md:86` now points to it; `README.md`, `README-ca.md`, `README-es.md` "AI-Ready Documentation" section bumped from 13 to 14 thematic documents; `knowledge/{en,ca,es}/README.md` "Related documentation" list extended). Addresses internal security-review finding `AUD-INT-001 §2.11`: the informal threat model at `SECURITY.md:3-15` is now a reviewed artefact — 8 trust boundaries (browser / CLI / Qdrant embedded / Ollama daemon / Hugging Face / filesystem / macOS Keyring / LAN-bootstrap), 6 asset categories, full STRIDE matrix with per-boundary mitigations citing `file:line` against the current codebase (after the integrity hardening), explicit out-of-scope enumeration (shell access, nation-state, multi-tenant, Python supply-chain pre-bundle, Apple firmware, malicious IDE extensions, offline `storage/` copies), honest residual-risks section (jailbreak speed-bump is not a safety classifier, `/ui/*` CSRF exemption trades cookie-CSRF for `X-API-Key`, loopback TLS is not enforced, no automated CVE tracking between releases, no external pen-test), and a LINDDUN privacy appendix covering MEM_SAVE exfiltration via prompt injection. Doc-only change — no code, no tests, no behaviour modification.
...
above for the SHA256 weight pinning, the STRIDE threat model, and the security
hardening fixes that landed since.
...
- No public release tag was cut for 1.0.2-beta; the version was used internally
  during the integrity-hardening and threat-model work and is documented here for chronological
...
Pre-release consolidated: Bug #18 MEM_DELETE fix + Bug #19 (4 sub-bugs) + offline-install bundle + Apple Silicon / macOS 14 narrowing + thinking toggle. Last P0 blockers closed before v1.0.
...
- **Thinking toggle endpoint**: `PATCH /ui/session/{id}/thinking`
