...
- **Test coverage `Qwen3VLForConditionalGeneration`** (`plugins/mlx_module/tests/test_multimodal.py`). `_VLM_ARCHITECTURES` already listed `Qwen3VLForConditionalGeneration` but had no explicit test case — the architecture was exercised only indirectly. Added `test_vlm_qwen3_vl_returns_true` to `TestDetectVlmCapability`, closing the gap between the safelist and the regression suite. Suite: 22 passed (was 21). Sprint v1.0.4-beta Fase 2. Catalog extension (MLX path for Qwen VL models) deferred: `mlx-community/Qwen3-VL-*` availability could not be verified without network access at time of authoring — tracked as Fase 2B pending.
