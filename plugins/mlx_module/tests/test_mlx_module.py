...
async def test_mlx_module_metadata():
    ...
    assert module.metadata.version == "0.9.1"
...
async def test_mlx_module_initialize_no_metal():
    ...
    with patch("plugins.mlx_module.core.config.MLXConfig.is_metal_available", return_value=False):
        module = MLXModule()
...
async def test_mlx_module_get_router():
    ...
    with patch("plugins.mlx_module.core.config.MLXConfig.is_metal_available", return_value=True), \
