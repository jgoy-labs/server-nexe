...
# nexe_flow is the (not-yet-shipped) workflow engine. Import it optionally so a
# real installation without it can still import this module without crashing;
# the workflow node simply stays unavailable until nexe_flow ships. Tests inject
# a mock nexe_flow in conftest.py, so this import succeeds there unchanged.
try:
...
if not NEXE_FLOW_AVAILABLE:
  # nexe_flow (the workflow engine) is not installed → the OllamaNode workflow
  # node cannot be defined (it subclasses nexe_flow's Node). Expose a None
  # sentinel so importers can degrade gracefully instead of crashing.
  OllamaNode = None  # type: ignore[assignment,misc]
else:
  class OllamaNode(Node):  # type: ignore[no-redef]
    """
    Node that calls local Ollama to generate text with LLMs.

    Inputs:
      - prompt (string, required): Text prompt for the LLM
      - model (string, optional): Model to use (default: "llama3.2")
      - temperature (number, optional): Creativity 0.0-2.0 (default: 0.7)
      - max_tokens (number, optional): Maximum response tokens (default: None)
    ...
    def get_metadata(self) -> NodeMetadata:
      ...
      return NodeMetadata(
        ...
        name="Ollama Chat",
        version="1.0.0",
        description="Generates text using local Ollama (llama, mistral, etc.)",
        category="llm",
        inputs=[
          NodeInput(
            name="prompt",
          ...
          NodeInput(
            ...
            required=False,
            default="llama3.2",
            description="Ollama model (llama3.2, mistral, etc.)"
          ),
          NodeInput(
            name="temperature",
            type="number",
    ...
    async def execute(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
      ...
      Returns:
        Dict with response, model_used, tokens
