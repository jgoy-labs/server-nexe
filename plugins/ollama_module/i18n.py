...
Version: 0.8
Author: Jordi Goy
Location: plugins/ollama_module/i18n.py
Description: Minimal i18n helper for Ollama plugin CLI/runtime messages.
────────────────────────────────────
"""
...
_i18n_singleton: Optional["I18nHelper"] = None
...
def _find_config_path() -> Optional[Path]:
  ...
  for parent in current.parents:
    candidate = parent / "personality" / "server.toml"
    if candidate.exists():
      return candidate
    candidate = parent / "server.toml"
    if candidate.exists():
      return candidate
  return None
...
def _load_language(lang: str) -> Dict[str, Any]:
  ...
  data: Dict[str, Any] = {}
  try:
    path = Path(__file__).resolve().parent / "languages" / lang / "common.json"
    if path.exists():
...
class I18nHelper:
  """Simple i18n helper exposing .t() like the core i18n services."""
