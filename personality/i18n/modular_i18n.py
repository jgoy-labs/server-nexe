...
class ModularI18nManager:
  """Modular internationalisation manager for server-nexe."""
  ...
  def _load_translation_file(self, file_path: Path) -> bool:
    """Load a specific translation file for the current language"""
    return self._load_translation_file_for_lang(file_path, self.current_language)

  def _load_translation_file_for_lang(self, file_path: Path, language: str) -> bool:
    """Load a specific translation file for a given language"""
    try:
      ...
      if prefix in data:
        self.translations[language][prefix] = data[prefix]
      else:
        self.translations[language][prefix] = data
