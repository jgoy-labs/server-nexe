  ...
  def _chunk_by_sentences(self, text: str) -> List[str]:
    ...
    for sentence in sentences:
      ...
      if len(current) + len(sentence) + 1 <= self.config["max_chunk_size"]:
        ...
        # Apply overlap from previous chunk (capped so total stays within max)
        overlap_size = self.config.get("chunk_overlap", 200)
        max_size = self.config["max_chunk_size"]
        if chunks and overlap_size > 0:
            # Cap overlap so prefix + sentence doesn't exceed max_chunk_size
            effective_overlap = min(overlap_size, max(0, max_size - len(sentence) - 1))
            if effective_overlap > 0:
                prefix = chunks[-1][-effective_overlap:] + " "
            else:
                prefix = ""
        else:
            prefix = ""
        # Hard-split sentences larger than max_chunk_size
