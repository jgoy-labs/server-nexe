...
if [ -d "venv" ] || [ -f ".env" ] || [ -d "storage" ]; then
    ...
    if [[ ${response:-} =~ ^[YySs]$ ]]; then
        ...
        # Ollama lifecycle managed by server — not killing external processes
        ...
        rm -f .qdrant-initialized 2>/dev/null || true
